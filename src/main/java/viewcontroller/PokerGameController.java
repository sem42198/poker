...
import model.BluffingAI;
import model.ConservativeAI;
import model.RiskTakerAI;
...
public class PokerGameController {
     ...
     * @param humanMoney Amount of money the human player starts with
     * @param aiMoney Array with amounts of moey each AI will start with
     * @param lastFirstPlayer Represents which player's turn it will be
     * @param aiTypes Array with ints representing which AI type each AI is
     */
    public PokerGameController(PokerGame gameView, String name, int humanMoney,
        int[] aiMoney, int lastFirstPlayer, int[] aiTypes) {

        this.gameView = gameView;

        this.lastFirstPlayer = lastFirstPlayer;

        human = new HumanPlayer(name, humanMoney);
        ais = new AIPlayer[3];
        ...
        for (int i = 0; i < ais.length; i++) {
            ...
            switch (type) {
            case 0:
                ais[i] = new BluffingAI("AI " + (i + 1), aiMoney[i]);
                players[i + 1] = ais[i];
                break;
            case 1:
                ais[i] = new ConservativeAI("AI " + (i + 1), aiMoney[i]);
                players[i + 1] = ais[i];
                break;
            case 2:
                ais[i] = new RiskTakerAI("AI " + (i + 1), aiMoney[i]);
                players[i + 1] = ais[i];
                break;
            default:
                ais[i] = new AIPlayer("AI " + (i + 1), aiMoney[i]);
                players[i + 1] = ais[i];
                break;
            }
        }
     ...
     * @param  gameView The PokerGame object to interact with
     * @param  name The human's name
     * @param money How much money players start with
     */
    public PokerGameController(PokerGame gameView, String name, int money) {
        this(gameView, name, money, new int[]{money, money, money}, -1,
            new int[]{-1, -1, -1});
    }
    ...
    /**
     * getter method for lastFirstPlayer
     * @return int representing which player went first last
     */
    ...
    /**
     * getter method for saveName, the name the game is saved under
     * @return The name the game is saved under
     */
    ...
    /**
     * setter method for saveName
     * @param saveName the name the game is saved under
     */
    ...
    private void takeAiTurn(AIPlayer ai) {
        ...
        ai.figurePokerHand(board.getCards());
        HandType handType = ai.getPokerHand().getType();
        Turn t = ai.getTurn(minBet, maxBet, board.getCards(), handType);
        think(1000, 400);
